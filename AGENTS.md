...
## Project overview
`ai-engineering` is a governance floor for AI coding agents: a single Bun-compiled binary (`ai-eng`) that installs governance (`init`), enforces guards through hooks (`chain`), and proves completeness with receipts (`spec run`). The binary IS the payload — every skill and template travels inside it (regenerate with `bun scripts/gen-assets.ts`). Product spec: `docs/blueprint.html` (v17, §-numbered).
...
- `bun run lint` / `bun run typecheck` — oxlint / oxlint type-aware. `typecheck` = `oxlint --type-aware --type-check`, the single type gate (lint + type-aware + compiler check).
