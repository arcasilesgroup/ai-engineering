...
For every finding that survived Phase 3 validation, produce a structured JSON object conforming to the schema defined in `report-schema.json` (in the `references/` directory next to this file — read it via the Read tool before writing output). Write the result to `<output-dir>/findings.json`.
