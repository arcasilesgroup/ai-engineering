...
description: >-
  Authors and revises Agent Behavior BEHAVIOR.md specs that capture recurring,
  judgeable agent conduct. Use when creating, reviewing, splitting, grouping,
  or calibrating behavior specs, or when translating trace failures and runtime
  guidance into durable behaviors. Trigger for "write a behavior spec", "turn
  this failure into a behavior", "calibrate this behavior", "BEHAVIOR.md". Not
  for merely scoring a completed trajectory — use /ai-verify — nor for writing
  a runtime prompt.
license: Apache-2.0
---
...
Create `.agents/behaviors/<name>/BEHAVIOR.md` (in ai-engineering: at the repository
root, not under `.ai-engineering/`; see **The ai-engineering seam**) with the required
frontmatter:
...
1. Output lives at `.agents/behaviors/<name>/BEHAVIOR.md` — at the ROOT of the
   repository, not under `.ai-engineering/`, because half the world reads it: human
   reviewers, eval judges, other tools. ai-eng only validates. This is an on-demand
