---
name: rtk
description: Route shell commands through rtk to cut output tokens.
version: 0.1.0
author: autometa, Hermes Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [rtk, cli, shell, tokens, output-filtering]
---
...
Optional but recommended — the Hermes plugin rewrites `terminal` commands automatically, so no per-command effort is needed:

```bash
rtk init --agent hermes     # writes ~/.hermes/plugins/rtk-rewrite/, enables it in config
rtk init --show             # verify what's installed
```

Restart Hermes afterward. Hand-prefixing stays correct either way — rtk never double-wraps a command that already starts with `rtk`.
