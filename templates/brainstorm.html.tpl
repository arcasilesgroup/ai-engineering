...
<title>Brainstorm · {{milestone}}</title>
<style>
:root{
...
h3{font-size:var(--fs-h3);line-height:var(--lh-h3);letter-spacing:var(--ls-h3);font-weight:640;margin:var(--s6) 0 var(--s3)}
...
<header class="hero">
  ...
  <div class="stamp">Brainstorm · {{date}} · {{status}}</div>
  <h1>{{title}}</h1>
...
<!-- Add sections here using the standard components:
     ...
     Standard sections for brainstorm:
     - The idea in plain words (read-back)
     - Why this matters
     - Who it's for
     - What exists today
...
<footer>
  <span class="x">{</span>ai<span class="x">}</span> Engineering · Brainstorm · {{milestone}}<br>
  {{footer}}
  <a href="#main">↑ top</a>
</footer>
</body>
