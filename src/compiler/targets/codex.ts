...
function buildCodexMd(ir: IntermediateRepresentation): string {
  ...
  sections.push(
    `- Never commit directly to: ${ir.config.branches.protectedBranches.join(", ")}`,
  );
  sections.push(
    "- Never commit secrets, credentials, or sensitive files (`.env*`, `*.pem`, `*.key`)",
  );
  ...
  sections.push("");
  sections.push(
    `- Default: \`${ir.config.branches.defaultBranch}\` (protected)`,
  );
  sections.push(
    `- Integration: \`${ir.config.branches.developBranch}\` (protected)`,
  );
  sections.push("- Features: `feature/*` or `dev/*`");
  sections.push("- Fixes: `fix/*`");
  sections.push(
    "- Hotfixes: `hotfix/*` (can push, must PR to default + develop)",
  );
  sections.push("");
