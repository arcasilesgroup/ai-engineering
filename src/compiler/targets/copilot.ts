...
function buildCopilotInstructions(ir: IntermediateRepresentation): string {
  ...
  sections.push(
    "- Never commit `.env*`, `*.pem`, `*.key`, `credentials*`, or files containing secrets",
  );
  ...
  sections.push("");
  sections.push(
    "- Feature branches: `feature/<description>` or `dev/<name>/<description>`",
  );
  ...
  sections.push(
    "1. Security: No OWASP top 10 vulnerabilities, no hardcoded secrets",
  );
  ...
  sections.push(
    "5. Performance: No unnecessary re-renders, N+1 queries, or blocking calls",
  );
  sections.push("");
...
export function writeCopilotOutput(
  _output: CopilotOutput,
  projectRoot: string,
): void {
