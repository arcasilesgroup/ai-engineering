...
import { readFile, fileExists, resolvePath } from "../utils/filesystem.js";
...
function initHelpers(): void {
  ...
  Handlebars.registerHelper(
    "join",
    function (arr: unknown[], separator: string) {
...
export function resolveTemplatePath(templateName: string): string {
  ...
  const templatePath = resolvePath(root, "templates", templateName);
  if (fileExists(templatePath)) return templatePath;

  logger.debug(`Template path tried: ${templatePath} (package root: ${root})`);
