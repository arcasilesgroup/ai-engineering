...
import { promptLevelSelect } from '../prompts/level-select.js';
import type { Stack, IDE, EnforcementLevel, Config } from '../../utils/config.js';
...
export async function initCommand(options: InitOptions): Promise<void> {
  ...
  if (fileExists(resolvePath(projectRoot, '.ai-engineering/config.yml'))) {
    logger.warn('ai-engineering is already initialized in this project.');
    logger.info("Use 'npx ai-engineering update' to update, or delete .ai-engineering/ to re-initialize.");
    return;
  }
  ...
  if (options.yes) {
    ...
    level = (options.level as EnforcementLevel) ?? 'standard';
  ...
  const platformResult = runPlatformCheck(stacks, level, platform);
  ...
  if (ides.includes('claude-code')) generatedFiles.push('CLAUDE.md', '.claude/commands/*', '.claude/settings.json');
  if (ides.includes('copilot')) generatedFiles.push('.github/copilot-instructions.md');
  if (ides.includes('codex')) generatedFiles.push('codex.md');
  if (level !== 'basic') generatedFiles.push('lefthook.yml', '.gitleaks.toml');
