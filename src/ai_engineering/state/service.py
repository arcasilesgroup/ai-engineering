...
def load_install_state(state_dir: Path) -> InstallState:
    """Load the install state from ``install-state.json``.

    spec-148 P4 (files-only): ``install-state.json`` is the writable
    single source of truth again — this reverses the spec-125 cutover
    that moved it into the state.db ``install_state`` singleton row.
    Returns a default :class:`InstallState` when the file is absent
    (first install). A structurally-legacy payload (pre-spec-101 shape)
    is migrated in place once.

    Args:
        state_dir: The ``.ai-engineering/state/`` directory holding
            ``install-state.json``.

    Returns:
        Parsed ``InstallState``, or a default instance when the file is
        absent.
    """
...
def save_install_state(state_dir: Path, state: InstallState) -> None:
    """Persist *state* to ``install-state.json`` (spec-148 P4 files-only).

    Reverses the spec-125 cutover: ``install-state.json`` is the writable
    single source of truth. Serialization matches the legacy-migration
    writer (``model_dump_json(indent=2)``) so the on-disk shape is stable.
    Creates the state directory when needed (older tests assume the
    directory exists post-call).
    ...
    logger.debug("install-state.json written at %s", path)
