...
def build_context_pack(
    ...
    *,
    ...
    manifest = ContextPackManifest(
        schemaVersion=CONTEXT_PACK_SCHEMA_VERSION,  # ty:ignore[unknown-argument]
        taskId=task.id if task else task_id,  # ty:ignore[unknown-argument]
        sources=_dedupe_sources(sources),
        ceilings=ContextPackCeilings(maxSources=max_sources, maxInlineChars=max_inline_chars),  # ty:ignore[unknown-argument]
        regenerationInputs=[
...
def classify_learning_artifact(path: str) -> LearningFunnelArtifact:
    ...
    return LearningFunnelArtifact(
        ...
        provenanceRef=normalized,  # ty:ignore[unknown-argument]
    )
...
def promote_learning_artifact(
    ...
    *,
    ...
    return LearningFunnelArtifact(
        ...
        canonicalDestination=destination,  # ty:ignore[unknown-argument]
        provenanceRef=artifact.provenance_ref or artifact.path,  # ty:ignore[unknown-argument]
        backlinkRef=backlink_ref,  # ty:ignore[unknown-argument]
    )
    ...
    *,
    ...
    sources.append(
        ContextPackSource(
            ...
            inlineChars=0,  # ty:ignore[unknown-argument]
        )
    )
