...
def _check_manifest_coherence(target: Path, report: IntegrityReport, **_kwargs: object) -> None:
    ...
    # they have moved to IDE-specific directories (.claude/, .codex/, .agents/).
    ownership_dirs = [
        ("reference", "framework_managed"),
...
def _task_to_capability_packet(task: TaskLedgerTask) -> CapabilityTaskPacket:
    ...
    return CapabilityTaskPacket(
        ...
        mutationClasses=mutation_classes or infer_mutation_classes(task.write_scope),  # ty:ignore[invalid-argument-type]
        writeScope=task.write_scope,
        toolRequests=tool_requests,  # ty:ignore[invalid-argument-type]
        provider=provider,
