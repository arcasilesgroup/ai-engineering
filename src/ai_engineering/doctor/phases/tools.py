...
def _fix_tools_required(
    ...
    *,
    ...
    if not missing:
        # No missing tools (incl. the fresh-checkout no-manifest case): nothing
        # to install, so the fix path closes cleanly and auto-remediation can
        # reach exit 0. The check phase already surfaces the actionable
        # "manifest not found; run 'ai-eng install'" WARN (spec-101 G-1, #510).
        return CheckResult(
            name=cr.name,
