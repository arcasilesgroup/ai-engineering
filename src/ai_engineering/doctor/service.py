...
def diagnose(
    ...
    *,
    ...
    # Its presence is the install-completed signal; its absence means
    # diagnose() falls into pre-install mode. Tests that need pre-install
    # behaviour intentionally leave state_dir without the file.
    state_dir = target / ".ai-engineering" / "state"
    install_state = None
    if (state_dir / "install-state.json").is_file():
        try:
