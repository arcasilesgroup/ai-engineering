...
emit_event() {
  ...
  if [ -w "$(dirname "$EVENTS_FILE")" ] || mkdir -p "$(dirname "$EVENTS_FILE")" 2>/dev/null; then
    printf '{"component":"scheduled.simplify-sweep","kind":"framework_operation","operation":"simplify_sweep_scheduled_run","outcome":"%s","detail":%s,"timestamp":"%s","schemaVersion":"1.0","source":"scheduled","engine":"cron","project":"ai-engineering"}\n' \
