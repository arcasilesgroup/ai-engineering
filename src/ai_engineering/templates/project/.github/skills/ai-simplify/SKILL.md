...
**Calls**: the `ai-simplify` agent (`.github/agents/simplify.agent.md`) via the Agent tool with the operator-chosen scope; validation runs after each edit; the agent rolls back on test failure.

**See also**:
- `.github/skills/ai-simplify-sweep/SKILL.md` — scheduled wrapper with draft-PR side effect.
