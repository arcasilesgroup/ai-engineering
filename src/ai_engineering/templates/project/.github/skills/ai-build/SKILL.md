...
3. **Execute kernel** — `.github/skills/_shared/execution-kernel.md`. Wrap each task (Sub-flow 1 dispatch → 2 build self-validation TDD RED/GREEN/REFACTOR → 3 artifact collection → 4 board sync). Update `plan.md` immediately as each task reaches a terminal state — never defer checkbox/status writes to phase or spec end.
