...
User: "optimize the description for /ai-governance" → `/ai-prompt-tune --skill ai-governance`. Reads `.github/skills/ai-governance/SKILL.md`, rewrites the description with explicit triggers + negative scoping, presents before/after, applies on approval, runs `sync_mirrors module`.
