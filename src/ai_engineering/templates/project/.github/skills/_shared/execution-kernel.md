...
- `.github/skills/ai-dispatch/SKILL.md` -- single-plan execution; consumer wraps the kernel per-task and runs Phase 5 quality + Phase 6 deliver after the loop completes.
...
- `.github/skills/ai-run/SKILL.md` -- backlog execution; consumer wraps the kernel per-item with bounded `ai-build` packets, integration gates after every promotion, and `ai-pr` for delivery.
