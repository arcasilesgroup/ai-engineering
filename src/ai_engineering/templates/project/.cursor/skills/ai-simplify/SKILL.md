...
3. **Dependency preflight** — verify `.cursor/agents/ai-simplify.mdc` exists. STOP and report the exact missing path if absent.
...
**Calls**: the `ai-simplify` agent (`.cursor/agents/ai-simplify.mdc`) via the Agent tool with the operator-chosen scope; validation runs after each edit; the agent rolls back on test failure.

**See also**:
- `.cursor/skills/ai-simplify-sweep/SKILL.md` — scheduled wrapper with draft-PR side effect.
