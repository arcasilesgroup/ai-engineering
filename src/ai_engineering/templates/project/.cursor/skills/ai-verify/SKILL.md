...
2. **Dependency preflight** — verify `handlers/verify.md` plus the `.cursor/agents/internal/verifier-*.md` files the selected mode needs exist (`normal`/`--full` need deterministic + acceptance; single modes need only their specialist). STOP and report the exact missing path — never improvise.
...
Dispatch the `ai-verify` agent for any merge-readiness check, scan, or evidence-backed claim over ≥ 1 changed file. Each specialist runs in its own context window via the Agent tool. `.cursor/agents/ai-verify.mdc` is the orchestrator handle; the procedural contract lives here.
