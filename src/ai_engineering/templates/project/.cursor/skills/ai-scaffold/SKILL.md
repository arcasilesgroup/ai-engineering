...
- [ ] File at correct path (`.cursor/skills/ai-<name>/SKILL.md` or `.cursor/agents/ai-<name>.mdc`)
...
Format reference: .cursor/skills/ai-security/SKILL.md or .cursor/skills/ai-review/SKILL.md
```
...
3. **Scaffold** `.cursor/agents/ai-<name>.mdc` — frontmatter (CSO description, `model`, `tools` whitelist, dispatch-source comment) plus body: Identity (role/experience/specialization), Mandate (owns/does-not-own), Capabilities (read-only/read-write/paths), Behavior (modes/procedures), Output Contract, Boundaries (limits/escalation), Self-challenge protocol.
