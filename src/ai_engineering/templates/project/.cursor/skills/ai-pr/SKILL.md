...
`/ai-pr` is the canonical chain step `brainstorm → plan → build → pr` — it runs the commit pipeline internally; the operator does NOT run `/ai-commit` first. `/ai-commit` is the standalone off-chain skill for WIP-only commits with no PR.
...
Total wall-clock = `max(docs, pre-push)`, NOT `sum`. Docs subagents and the pre-push gate run in parallel; the PR description stays coherent because docs are produced and staged BEFORE PR creation.
...
- When `summary:` is present in the spec frontmatter the PR body Summary section is composed **without any LLM call**.
- Legacy specs predating the field fall back to `--bullets-prompt "<llm-bullets>"`; emit an advisory warning to backfill the spec frontmatter `summary:` field. Do NOT pass `--bullets-prompt` when the active spec already declares `summary:`.
- The PR body itself (`## Summary` and every other composed section) is a deterministic machine artifact — a **carve-out** — emitted exactly as `pr_body_compose.py` renders it; the value lens applies only to the operator-facing status report (Step 16), never to the PR body, commit messages, or gate verdicts.
...
User: "I'm ready for review on this branch" → `/ai-pr`. Runs the commit pipeline (0-6), pre-push gates, generates the PR body from the spec's Summary + Test Plan, opens via `gh pr create`, transitions board state, watches CI. `--draft` runs the same pipeline but opens with `--draft` and skips the review request until `/ai-pr review`.
