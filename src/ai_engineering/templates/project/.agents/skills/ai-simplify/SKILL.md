...
3. **Dependency preflight** — verify `.agents/agents/ai-simplify.md` is on disk. STOP and report the exact missing path if absent.
...
**Calls**: the `ai-simplify` agent (`.agents/agents/ai-simplify.md`) via the
...
**See also**:
- `.agents/skills/ai-simplify-sweep/SKILL.md` — scheduled wrapper with draft-PR side effect (different cadence and contract).
