...
Read the target skill's SKILL.md. If `$ARGUMENTS` is `all`, list skills from `.agents/skills/` and process them in priority order: workflow first (plan, dispatch, review, verify, commit, pr), then enterprise, then meta.
...
```
The skill at .agents/skills/<name>/SKILL.md was just rewritten based on pain
