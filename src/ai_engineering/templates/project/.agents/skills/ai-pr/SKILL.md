...
If `.ai-engineering/specs/spec.md` is non-placeholder, resolve the canonical numeric id from its frontmatter `spec:` field (e.g. `spec-153` — NOT the human-readable slug; numeric `spec-NNN` is the one canonical identity per spec-153 D-153-01) and run the shared spec-consolidation handler at `.agents/skills/_shared/consolidate-spec.md` **on the feature branch** (it invokes `python .ai-engineering/scripts/spec_lifecycle.py mark_shipped <spec-NNN> N <branch>` to walk the state forward to SHIPPED, append the canonical 7-col `_history.md` row, snapshot spec.md+plan.md into `archive/spec-NNN-<slug>/`, clear spec.md+plan.md to placeholders, and emit the `framework_operation` audit event). Then `git add` the archived + cleared files, commit `chore(spec-NNN): consolidate (archive + clear live slot)`, and push to the same branch so the open PR updates.
