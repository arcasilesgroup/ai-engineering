...
Normative shared rules are defined in `.agents/skills/ops/SKILL.md` under **Shared Rules (Canonical)** (`OPS-R1..OPS-R5`, `OPS-B1`). The agent references those rules instead of redefining them.
...
1. **Apply shared operational rules** -- execute `OPS-R1..OPS-R5` from `.agents/skills/ops/SKILL.md`.
...
- `.agents/skills/ops/SKILL.md` -- operational automation (run, incident, status)
