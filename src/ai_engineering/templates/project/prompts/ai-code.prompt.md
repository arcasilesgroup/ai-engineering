...
> **Telemetry** (cross-IDE): run `ai-eng signals emit skill_invoked --actor=ai --detail='{"skill":"code"}'` at skill start. Fail-open -- skip if ai-eng unavailable.
...
- `.github/agents/build.agent.md` -- the agent that invokes this skill.
- `.github/prompts/ai-debug.prompt.md` -- systematic bug diagnosis (use instead for root-cause investigation).
- `.github/prompts/ai-refactor.prompt.md` -- structural changes (use instead for move/rename/split).
- `.github/prompts/ai-simplify.prompt.md` -- complexity reduction (use instead for guard clauses, early returns).
