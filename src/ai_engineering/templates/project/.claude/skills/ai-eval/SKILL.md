...
- `--skill-set`: skill-set mode — runs the optimizer over each skill's eval corpus under `evals/<skill>.jsonl` and gates pass@1 vs `evals/baseline.json`. Combine with `--regression` to fail on >5 pp pass@1 drop (sub-007 M6, D-127-07). Wired into `.github/workflows/skill-evals.yml` on PRs touching `.claude/skills/**`.
