...
- Command: `/ai-dashboard [engineer|team|ai|dora|health]`
- Context: need visibility into code quality, delivery velocity, or framework health.

> **Telemetry** (cross-IDE): run `ai-eng signals emit skill_invoked --actor=ai --detail='{"skill":"dashboard"}'` at skill start. Fail-open — skip if ai-eng unavailable.
