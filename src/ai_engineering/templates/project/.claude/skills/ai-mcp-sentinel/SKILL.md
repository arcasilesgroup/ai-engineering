...
- Before merging PRs touching `.claude/skills/`, `.codex/skills/`, `.gemini/skills/`, `.github/skills/`.
...
`ai-mcp-sentinel scan [--target <path-or-skill-name>]`
...
`ai-mcp-sentinel audit-update <skill-name>`
...
`ai-mcp-sentinel baseline set [--target <skill-name>|all]`
...
`/ai-security` adds CVE/SBOM/secrets; Sentinel adds coherence/rug-pull. `/ai-governance` consumes VERDE/ROJO verdicts. `/ai-platform-audit` verifies platform support; Sentinel verifies skill behavior. `prompt-injection-guard` hook (D-107-06) hot-path runtime; Sentinel cold-path counterpart.
