...
3. **Dependency preflight** — verify `reviewer-context.md`, `reviewer-validator.md`, plus required `.claude/agents/reviewer-*.md` files for the selected mode and detected diff scope (`frontend` conditional on UI work — covers React, hooks, animation, typography, forms, a11y). STOP and report exact missing path(s) — never paraphrase missing reviewer instructions inline.
