...
2. **Dependency preflight** — verify `.claude/skills/ai-verify/handlers/verify.md` plus required `.claude/agents/verifier-*.md` files exist for the selected mode (`normal` and `--full` both require deterministic + acceptance post-W3; individual modes require only the matching specialist). STOP and report exact missing path(s) — never improvise.
