...
2. If `--resume` flag: read `.ai-engineering/specs/autopilot/manifest.md` and jump to the Resume Protocol (Phase 6 handler).
...
Read `handlers/phase-decompose.md` and execute. Extract N independent concerns; abort if N < 3 (recommend `/ai-dispatch`); write sub-spec dirs `.ai-engineering/specs/autopilot/sub-NNN/` with `spec.md` + `plan.md` shells; write execution manifest to `.ai-engineering/specs/autopilot/manifest.md`.
...
Read `handlers/phase-implement.md` and execute. **Per-wave kernel**: see `.gemini/skills/_shared/execution-kernel.md`. Autopilot wraps the kernel per-wave -- dispatch the build agent per sub-spec in parallel within a wave (Sub-flow 1), run the build-verify-review loop per task (Sub-flow 2), collect Self-Reports + per-wave commits (Sub-flow 3), advance board state (Sub-flow 4). Cascade-block dependents of failed sub-specs.
...
Read `handlers/phase-deliver.md` and execute. Build Integrity Report from Self-Reports + quality audit; follow `/ai-pr` SKILL.md in full; cleanup `.ai-engineering/specs/autopilot/`; clear spec.md + plan.md; verify cleanup. Resume Protocol handles mid-pipeline re-entry via `--resume`.
...
| Flag         | Behavior                                                                                                                              |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| `--resume`   | Read `.ai-engineering/specs/autopilot/manifest.md`, determine pipeline state, re-enter at the correct phase/wave. Never re-executes completed phases. |
...
State transitions are recorded on disk in `.ai-engineering/specs/autopilot/manifest.md` -- never in agent memory. Any phase can be audited post-hoc.
...
- **Reads**: `.gemini/skills/_shared/execution-kernel.md`, `ai-verify/SKILL.md`, `ai-review/SKILL.md`, `ai-governance/SKILL.md`, `ai-pr/SKILL.md`, `ai-commit/SKILL.md`
