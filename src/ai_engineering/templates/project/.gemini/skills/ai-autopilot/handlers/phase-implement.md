...
- Manifest at `.ai-engineering/state/runtime/autopilot/manifest.md` contains a `## Execution DAG` section with wave assignments.
...
Read `.ai-engineering/state/runtime/autopilot/manifest.md`. Extract the `## Execution DAG` section. Parse:
...
1. **Sub-spec scope and exploration** -- from `.ai-engineering/state/runtime/autopilot/sub-NNN/spec.md` (Scope, Exploration, file ownership).
1b. **Sub-spec plan** -- from `.ai-engineering/state/runtime/autopilot/sub-NNN/plan.md` (task checkboxes).
...
After completing task T-N.K, edit `.ai-engineering/state/runtime/autopilot/sub-NNN/plan.md` to change `- [ ] T-N.K` to `- [x] T-N.K`.

#### 2d -- Agent Writes Self-Report

After completing (or failing) its plan tasks, each agent appends a `## Self-Report` section to `.ai-engineering/state/runtime/autopilot/sub-NNN/plan.md` using the Transparency Protocol:
...
After the wave commit, update `.ai-engineering/state/runtime/autopilot/manifest.md`:
...
- Self-Reports -- appended to each `.ai-engineering/state/runtime/autopilot/sub-NNN/plan.md`.
- Updated manifest -- `.ai-engineering/state/runtime/autopilot/manifest.md` with `implemented`, `blocked`, or `cascade-blocked` statuses per sub-spec, plus wave commit hashes.
