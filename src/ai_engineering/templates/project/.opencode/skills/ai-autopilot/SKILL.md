...
Dispatch the `ai-autopilot` agent when work has ≥3 independent concerns, touches ≥10 files, follows `/ai-brainstorm` approval (spec.md exists, not a placeholder), or runs any `--backlog`; smaller scope hands off to `/ai-build`. The agent handle is `.codex/agents/ai-autopilot.md`; the procedural contract lives in this SKILL.md.
