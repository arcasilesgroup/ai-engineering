...
Dispatch the `ai-autopilot` agent when work matches "When to Use" (≥3 concerns, ≥10 files, post-`/ai-brainstorm` approval, or any `--backlog`). For smaller scope, hand off to `/ai-build`. The agent file (`.codex/agents/ai-autopilot.md`) is the orchestrator handle; the procedural contract lives in this SKILL.md.
