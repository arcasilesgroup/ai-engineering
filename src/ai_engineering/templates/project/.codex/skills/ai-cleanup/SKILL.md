...
When invoked with `--consolidate-spec <slug>`, read `.codex/skills/_shared/consolidate-spec.md` and execute the shared handler: resolve the spec record, append the `_history.md` row via `spec_lifecycle.py mark_shipped`, clear `.ai-engineering/specs/spec.md` and `plan.md` to placeholders. Fail-open on missing script.
