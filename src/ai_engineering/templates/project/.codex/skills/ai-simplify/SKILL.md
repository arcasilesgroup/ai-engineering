...
3. **Dependency preflight** — verify `.codex/agents/ai-simplify.md` exists. STOP and report the exact missing path if absent.
...
**Calls**: the `ai-simplify` agent (`.codex/agents/ai-simplify.md`) via the Agent tool with the operator-chosen scope; validation runs after each edit; the agent rolls back on test failure.

**See also**:
- `.codex/skills/ai-simplify-sweep/SKILL.md` — scheduled wrapper with draft-PR side effect.
