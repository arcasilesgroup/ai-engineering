...
- `context/specs/_catalog.md` — auto-generated catalog of all specs (regenerated by `ai-eng spec catalog`).
- `context/specs/archive/` — completed specs moved here after merge.

### Spec Enriched Frontmatter

Spec `spec.md` frontmatter includes: `id`, `slug`, `status`, `created`, `size` (S/M/L/XL), `tags`, `branch`, `pipeline` (full/standard/hotfix/trivial), `decisions` (list of decision IDs).

Tasks `tasks.md` frontmatter includes: `spec`, `total`, `completed`, `last_session`, `next_session`. Counters are auto-corrected by `ai-eng spec verify`.

### Spec CLI Commands

- `ai-eng spec verify` — count checkboxes, auto-correct frontmatter, emit audit signal.
- `ai-eng spec catalog` — regenerate `_catalog.md` from all archived specs.
- `ai-eng spec list` — display active spec with progress percentage.
- `ai-eng spec compact [--dry-run] [--older-than 6m]` — remove bulky files from old archived specs.
- `ai-eng decision record <id> --context ... --decision ... [--spec ...] [--severity ...] [--category ...] [--expires ...]` — dual-write: JSON + audit log.
