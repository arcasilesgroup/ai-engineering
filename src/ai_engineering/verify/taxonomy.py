...
def build_seed_eval_pack() -> EvalScenarioPack:
    ...
    return EvalScenarioPack(
        ...
        baselineRef=".ai-engineering/evals/baselines/seed.json",  # ty:ignore[unknown-argument]
        scenarios=(
            EvalScenario(
...
def build_reliability_scorecard(
    *,
    ...
    return ReliabilityScorecard(
        ...
        latencyP95Ms=_percentile(latency_samples_ms, 95) if latency_samples_ms else None,  # ty:ignore[unknown-argument]
        retryCount=retry_count,  # ty:ignore[unknown-argument]
        provenanceRefs=provenance_refs,
    )
