...
export async function install(projectRoot: string, config: Config): Promise<void> {
  ...
  // Install CI/CD gates (strict only)
...
function collectFrameworkFiles(projectRoot: string, config: Config): string[] {
  ...
  for (const f of aiEngFiles) {
    if (!f.includes('/knowledge/') && !f.includes('/.version') && !f.includes('/.update-cache')) {
      files.push(f);
    }
  }
  ...
  if (config.ides.includes('claude-code')) {
    ...
    const commandFiles = listFiles(resolvePath(projectRoot, '.claude/commands'));
