...
def test_no_deprecated_skill_directories() -> None:
    ...
    survivors = sorted(
        slug for slug in _DEPRECATED_SKILLS if (_CANONICAL_SKILLS / slug).is_dir()
    )
...
def test_no_deprecated_agent_files() -> None:
    ...
    survivors = sorted(
        name for name in _DEPRECATED_AGENTS if (_CANONICAL_AGENTS / name).is_file()
    )
    assert not survivors, (
...
def test_renamed_skill_directories_exist() -> None:
    ...
    missing = sorted(
        slug for slug in _RENAMED_SKILLS
        if not (_CANONICAL_SKILLS / slug / "SKILL.md").is_file()
    )
    assert not missing, (
...
def test_renamed_agent_files_exist() -> None:
    ...
    missing = sorted(
        name for name in _RENAMED_AGENTS
        if not (_CANONICAL_AGENTS / name).is_file()
    )
    assert not missing, (
