"""RED skeleton for spec-107 G-6 (Phase 3) — platform-audit Checks 6/7/8.

Spec-107 D-107-05 extends `/ai-platform-audit` with three new checks:
...
def test_skill_md_documents_check_6() -> None:
    ...
    assert PLATFORM_AUDIT_SKILL.is_file(), (
        f"platform-audit SKILL.md missing: {PLATFORM_AUDIT_SKILL}"
    )
    text = PLATFORM_AUDIT_SKILL.read_text(encoding="utf-8")
...
def test_skill_md_advisory_severity_documented() -> None:
    ...
    assert PLATFORM_AUDIT_SKILL.is_file()
    text = PLATFORM_AUDIT_SKILL.read_text(encoding="utf-8").lower()
    assert "advisory" in text, (
        "platform-audit SKILL.md must document that the new checks are "
...
def test_skill_md_references_spec_107() -> None:
    ...
    assert "spec-107" in text, (
        "platform-audit SKILL.md missing `spec-107` reference for the new "
