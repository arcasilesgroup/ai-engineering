...
mirror surface so cross-IDE consumers see the same canonical kernel that
the .claude/ orchestrators delegate to. spec-201 D-201-04 collapsed the
skill trees to two, so the sole mirror surface is now ``.agents/``.
