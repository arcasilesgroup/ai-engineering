...
def test_capability_schema_is_closed_and_permission_distinct() -> None:
    ...
    assert len(allowed_ids) == len(set(allowed_ids)) == 18
...
def test_capabilities_toml_declares_exactly_eighteen_capabilities() -> None:
    schema = json.loads(SCHEMA_PATH.read_text(encoding="utf-8"))
...
def test_every_capability_says_which_phase_it_serves():
    ...
    The manifest is the only place all eighteen are enumerated, so the phase belongs there
