...
def test_hook_invocation_under_50ms(ai_eng_available: bool) -> None:
    ...
    p95 = _p95(samples)
    budget = _budget(SINGLE_HOOK_BUDGET_S * 10)
    # NOTE: 50 ms is aspirational; Python startup alone is 30-80 ms
    # cold, so we relax to 500 ms (10x) to catch *order-of-magnitude*
    # regressions, not micro-noise. Spec-122-d acceptance targets the
    # full pre-commit budget (1 s p95); this test is an early-warning.
    median = statistics.median(samples)
