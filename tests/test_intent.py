...
def test_skeleton_seed_is_write_once(tmp_path: Path, monkeypatch: Any) -> None:
    from ai_engineering import intent, skeletons

    corpus = json.loads(FIXTURE_PATH.read_text(encoding="utf-8"))
    canonical = _fixture_case(corpus, corpus["cases"][0])

    def repository(name: str, materialized: dict[str, Any]) -> Path:
        root = tmp_path / name
        root.mkdir()
        for file in materialized["repository"]["files"]:
            target = root / file["path"]
    ...
    direct = repository("direct", canonical)
    candidate = deepcopy(canonical["intent"])
    candidate["identity"]["title"] = "Governed delivery café"
    assert skeletons.seed_intent(direct, candidate).as_dict() == {"outcome": "PASS"}

    home = direct / ".ai" / "intent.md"
    expected = (
        json.dumps(candidate, allow_nan=False, ensure_ascii=False, indent=2, sort_keys=True) + "\n"
    ).encode()
    assert home.read_bytes() == expected
    assert home.stat().st_nlink == 1
    assert b"caf\xc3\xa9" in expected
    assert set(path.name for path in home.parent.iterdir()) == {"intent.md"}
    assert not (direct / "intent.md").exists()
    assert not (tmp_path / ".ai").exists()

    user_edit = b'{"now":"user-owned, even while invalid"}\n'
    home.write_bytes(user_edit)
    assert skeletons.seed_intent(direct, candidate).as_dict() == {
        "outcome": "INCOMPLETE",
        "code": "INTENT_ALREADY_EXISTS",
        "reason": "canonical Intent already exists",
    }
    assert home.read_bytes() == user_edit
    assert set(path.name for path in home.parent.iterdir()) == {"intent.md"}
    ...
    for case_id in (
        ...
        invalid = _fixture_case(corpus, case)
        root = repository(case_id, invalid)
        assert (
            skeletons.seed_intent(root, invalid["intent"]).as_dict() == case["expected_incomplete"]
        )
        assert not (root / ".ai").exists()

    linked_parent = repository("linked-parent", canonical)
    outside_directory = tmp_path / "outside-ai"
    outside_directory.mkdir()
    (linked_parent / ".ai").symlink_to(outside_directory, target_is_directory=True)
    assert skeletons.seed_intent(linked_parent, canonical["intent"]).outcome == "INCOMPLETE"
    assert not (outside_directory / "intent.md").exists()

    linked_file = repository("linked-file", canonical)
    linked_home = linked_file / ".ai"
    linked_home.mkdir()
    outside_file = tmp_path / "outside-intent.md"
    outside_file.write_bytes(b"outside stays untouched\n")
    (linked_home / "intent.md").symlink_to(outside_file)
    assert skeletons.seed_intent(linked_file, canonical["intent"]).outcome == "INCOMPLETE"
    assert outside_file.read_bytes() == b"outside stays untouched\n"

    real_root = repository("real-root", canonical)
    root_alias = tmp_path / "root-alias"
    root_alias.symlink_to(real_root, target_is_directory=True)
    assert skeletons.seed_intent(root_alias, canonical["intent"]).outcome == "INCOMPLETE"
    assert not (real_root / ".ai").exists()

    unsupported = repository("unsupported-filesystem", canonical)

    def unsupported_link(*args: Any, **kwargs: Any) -> None:
        raise NotImplementedError

    with monkeypatch.context() as patch:
        patch.setattr(skeletons.os, "link", unsupported_link)
        assert skeletons.seed_intent(unsupported, canonical["intent"]).as_dict() == {
            "outcome": "INCOMPLETE",
            "code": "INTENT_WRITE_FAILED",
            "reason": "canonical Intent could not be created",
        }
    assert not (unsupported / ".ai").exists()

    concurrent = repository("concurrent", canonical)
    workers = 16
    ready = Barrier(workers)
    real_validate = intent.validate

    def synchronized_validate(*args: Any, **kwargs: Any) -> intent.Validation:
    ...
    with monkeypatch.context() as patch:
        patch.setattr(intent, "validate", synchronized_validate)
        with ThreadPoolExecutor(max_workers=workers) as pool:
            results = list(
                pool.map(
                    lambda _: skeletons.seed_intent(concurrent, canonical["intent"]),
                    range(workers),
                )
            )
    assert [result.outcome for result in results].count("PASS") == 1
    assert all(
        result.outcome == "PASS" or result.code == "INTENT_ALREADY_EXISTS" for result in results
    )
    assert set(path.name for path in (concurrent / ".ai").iterdir()) == {"intent.md"}


def test_skeleton_seed_cleans_failed_publication(tmp_path: Path, monkeypatch: Any) -> None:
    from ai_engineering import intent, skeletons
    ...
    assert "one cooperative repository writer" in (skeletons.seed_intent.__doc__ or "")
    assert "not a security boundary" in (skeletons.seed_intent.__doc__ or "")

    def refuse_write(fd: int, payload: bytes) -> None:
        raise OSError("file write unavailable")

    def refuse_fsync(fd: int) -> None:
        raise OSError("file fsync unavailable")

    for operation, failure in (("write", refuse_write), ("fsync", refuse_fsync)):
        write_failure = repository(f"{operation}-failure")
        with monkeypatch.context() as patch:
            patch.setattr(skeletons.os, operation, failure)
            assert skeletons.seed_intent(write_failure, canonical["intent"]).as_dict() == {
                "outcome": "INCOMPLETE",
                "code": "INTENT_WRITE_FAILED",
                "reason": "canonical Intent could not be created",
            }
        assert not (write_failure / ".ai").exists()

    final_failure = repository("final-validation-failure")
    real_validate = intent.validate
    validations = 0

    def fail_final(*args: Any, **kwargs: Any) -> intent.Validation:
        nonlocal validations
        validations += 1
        if validations == 2:
            return intent.Validation(
                "INCOMPLETE", "INTENT_RELATION_STALE", "relation changed before publication"
            )
        return real_validate(*args, **kwargs)

    with monkeypatch.context() as patch:
        patch.setattr(intent, "validate", fail_final)
        assert skeletons.seed_intent(final_failure, canonical["intent"]).outcome == "INCOMPLETE"
    assert validations == 2
    assert not (final_failure / ".ai").exists()
