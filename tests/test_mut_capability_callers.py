...
the plural — one capability of seventeen has a caller, and that sentence sat in a note.
...
def test_the_count_of_capabilities_with_a_caller_is_published_and_does_not_drop():
    ...
    One of seventeen today, through `ai-report issue`. That gap is real and this does not
    ...
    assert len(known) == 17, f"{len(known)} capabilities are declared and the audit measured 17"
    assert taken, (
        "no capability has a caller at all. The manifest would then be seventeen declarations "
