...
Usage: SONAR_TOKEN=... python tests/quality_gate.py [project-key]
"""
...
API = "https://sonarcloud.io/api/qualitygates/get_by_project?project="
...
def live(project: str, token: str) -> dict[str, tuple[str, float]]:
    request = urllib.request.Request(API + project)
    request.add_header("Authorization", f"Bearer {token}")
    with urllib.request.urlopen(request, timeout=30) as answer:
        body = json.load(answer)
    found = {}
    for row in body.get("qualityGate", {}).get("conditions", []):
        try:
...
def main(project: str, token: str) -> int:
    want, have = declared(), live(project, token)
    problems = []
...
if __name__ == "__main__":
    key = sys.argv[1] if len(sys.argv) > 1 else "arcasilesgroup_ai-engineering"
