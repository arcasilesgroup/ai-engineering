...
Master advanced patterns for maximum productivity: parallel agents, plan mode, hooks, and multi-instance workflows.
...
Terminal 4: "Run security-scanner and quality-checker agents"
...
### PreToolUse Hook: Prevent Dangerous Commands
...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "Bash",
        "command": "echo $COMMAND | grep -q 'git push --force' && exit 1 || exit 0"
      }
    ]
  }
}
...
4. **Agents**: Run `build-validator` + `security-scanner` in parallel.
...
You've completed the AI Engineering Framework workshop. You now know how to:

- Install and configure the framework
- Use core commands for your daily workflow
- Set up quality gates and security scanning
- Configure CI/CD for GitHub Actions and Azure Pipelines
- Create custom commands, agents, and standards
- Use advanced patterns for maximum productivity

The framework is designed to grow with your team — add learnings, customize standards, and create new commands as your project evolves.
