...
Dispatch the `ai-review` agent for any narrative review over ≥ 1 changed file (PR, branch, diff, or path scope). Each specialist runs in its own context window via the host subagent primitive. `.claude/agents/ai-review.md` is the orchestrator handle; profiles, roster, output contract, and validator stage live here.
