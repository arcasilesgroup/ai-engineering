...
**Inline fallback** — subagent dispatch is the primary path. On a host without a subagent primitive, execute this skill by reading `.claude/agents/ai-onboard.md` inline and running the selected mode procedure in-context, sequentially; inline-sequential is the floor, not an alternate behaviour.
