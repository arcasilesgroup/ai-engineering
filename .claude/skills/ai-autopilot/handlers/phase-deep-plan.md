...
- Sub-spec directories exist at `.ai-engineering/specs/autopilot/sub-NNN/` with `spec.md` (Scope + `files:` frontmatter) and `plan.md` (Plan placeholder).
- Manifest exists at `.ai-engineering/specs/autopilot/manifest.md` with all sub-spec statuses set to `planning`.
...
1. Glob `.ai-engineering/specs/autopilot/sub-*/spec.md`. Collect the full list of sub-spec directories.
...
- N enriched sub-spec directories at `.ai-engineering/specs/autopilot/sub-NNN/`:
