...
- `.claude/skills/ai-build/SKILL.md` -- single-plan execution; consumer wraps the kernel per-task and runs Phase 5 quality + Phase 6 deliver after the loop completes.
...
- `.claude/skills/ai-autopilot/SKILL.md` `--backlog` mode -- absorbs the legacy `/ai-run` skill (D-127-12); consumer wraps the kernel per-item with bounded `ai-build` packets, integration gates after every promotion, and `ai-pr` for delivery.
