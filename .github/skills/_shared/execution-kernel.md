...
- `.github/skills/ai-build/SKILL.md` -- single-plan execution; consumer wraps the kernel per-task and runs Phase 5 quality + Phase 6 deliver after the loop completes.
- `.github/skills/ai-autopilot/SKILL.md` -- multi-spec autonomous execution; consumer wraps the kernel per-wave (parallel sub-specs within a wave, DAG-driven across waves) before Phase 5 quality loop and Phase 6 deliver.
- `.github/skills/ai-autopilot/SKILL.md` `--backlog` mode -- absorbs the legacy `/ai-run` skill (D-127-12); consumer wraps the kernel per-item with bounded `ai-build` packets, integration gates after every promotion, and `ai-pr` for delivery.
