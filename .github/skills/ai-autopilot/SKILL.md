...
**Step 4 — IMPLEMENT** (`handlers/phase-implement.md`): per-wave kernel from `.github/skills/_shared/execution-kernel.md`. Dispatch build agents per sub-spec in parallel within a wave; run build-verify-review per task; collect Self-Reports + per-wave commits. Cascade-block dependents of failed sub-specs.
...
Dispatch the `ai-autopilot` agent when the work matches the "When to Use" criteria above (≥3 concerns, ≥10 files, post-`/ai-brainstorm` approval, or any backlog with `--backlog`). For smaller scope, hand off to `/ai-build` directly. The agent file (`.github/agents/autopilot.agent.md`) is the orchestrator handle; the procedural contract lives in this SKILL.md.
