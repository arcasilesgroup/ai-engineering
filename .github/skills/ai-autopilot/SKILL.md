...
**Step 4 — IMPLEMENT** (`handlers/phase-implement.md`): per-wave kernel from `.github/skills/_shared/execution-kernel.md`. Dispatch build agents per sub-spec in parallel within a wave; each task self-validates via TDD; wave-end guard advisory remains for governance. Collect Self-Reports + per-wave commits. Cascade-block dependents of failed sub-specs.
...
Dispatch the `ai-autopilot` agent when work has ≥3 independent concerns, touches ≥10 files, follows `/ai-brainstorm` approval (spec.md exists, not a placeholder), or runs any `--backlog`; smaller scope hands off to `/ai-build`. The agent handle is `.github/agents/autopilot.agent.md`; the procedural contract lives in this SKILL.md.
