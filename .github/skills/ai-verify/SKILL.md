...
2. **Dependency preflight** — verify `handlers/verify.md` plus the `.github/agents/internal/verifier-*.md` files the selected mode needs exist (`normal`/`--full` need deterministic + acceptance; single modes need only their specialist). STOP and report the exact missing path — never improvise.
