...
2. **Dependency preflight** — verify `.github/skills/ai-verify/handlers/verify.md` plus required `.github/agents/internal/verifier-*.md` files exist for the selected mode (`normal` and `--full` both require deterministic + acceptance post-W3; individual modes require only the matching specialist). STOP and report exact missing path(s) — never improvise.
