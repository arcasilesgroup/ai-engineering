...
1. [ ] <!--t:044-a1--> **Family (a): delete the orphan layer** —
   **files** `src/ai_engineering/{constellation,decision_fw,decision_boundary,intake,
...
2. [ ] <!--t:044-b1--> **Family (b): one digest-pinned policy loader** —
   **files** `src/ai_engineering/` modules carrying `_EXPECTED*_DIGEST` readers
...
3. [ ] <!--t:044-b2--> **Family (b): `acceptance._parse_legacy` delegates to
   `text.flat_yaml`** —
   **files** `src/ai_engineering/acceptance.py` (the shared home absorbs the container
...
4. [ ] <!--t:044-b3--> **Family (b): one ls-files reader + one git wrapper +
   `wiring.cli_answers` behind `doctor`** —
   **files** `src/ai_engineering/{doctor,contract,evidence,madr}.py` (ls-files),
...
5. [ ] <!--t:044-c1--> **Family (c): delete zero-caller dead weight and fold the
   one-caller relics** —
   **files** `src/ai_engineering/spec.py` (`self_contained`+`_LEAKS` die with
...
6. [ ] <!--t:044-d1--> **Family (d): kill the `noqa: E402` idiom in test modules** —
   **files** the 15 surviving test modules carrying `sys.path.insert(ROOT/"src")` +
...
7. [ ] <!--t:044-d2--> **Family (d): shared test fixtures and the schema reader** —
   **files** `tests/conftest.py` (promote `home`/`machine`/`repo` fixtures),
...
8. [ ] <!--t:044-final--> **Block close: the gate over the whole branch** —
