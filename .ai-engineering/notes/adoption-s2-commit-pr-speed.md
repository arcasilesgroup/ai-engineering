...
> **PROMOTED 2026-04-26**: This brainstorm note has been promoted to `.ai-engineering/specs/spec.md` as **spec-104** (renumbered from initial draft `spec-102` to avoid collision with historical `spec-102` in `_history.md`). All `D-102-XX` references below should be read as `D-104-XX` in the canonical spec. Plan generated at `.ai-engineering/specs/plan.md`. Original brainstorm finding (2026-04-24) preserved at the bottom of this file.

# Spec 102 — Commit/PR Pipeline Speed: Single-Pass Collector + Memoization + Bounded Watch
*(Note: actual canonical ID is spec-104; see banner above)*

## Summary

`/ai-commit` y `/ai-pr` son medium-slow a slow porque ejecutan secuencialmente 6 checks (`ruff format` → `ruff check` → `gitleaks staged` → docs gate LLM → `ai-eng validate` → `ai-eng spec verify`), repiten el bloque pre-push completo (~30-120s `semgrep` + `pytest` + `pip-audit` + `ty`) que la matriz CI ya cubre, dispachan dos subagentes `/ai-docs` síncronos antes de crear el PR, llaman `ai-eng spec verify` dos veces sin invalidación, y mantienen un watch loop sin tope global de iteraciones (`fix_attempts>=3` por check pero `iteration_count` ilimitado). Cero memoización: cada re-run ejecuta todo desde cero aunque ningún input haya cambiado. Verbosity acumulada en 532 líneas (`ai-commit/SKILL.md` 126 + `ai-pr/SKILL.md` 221 + `ai-pr/handlers/watch.md` 185) duplicando reglas que ya viven en `CLAUDE.md` y `.ai-engineering/contexts/languages/`. Resultado: tiempo total `/ai-pr` ~3-5 min en cold-cache, fricción de adopción en GitHub Copilot, Codex y Gemini idéntica porque la lentitud está en la capa CLI/hook (IDE-agnóstica). Spec-102 reescribe el pipeline como dos olas (fixers serial → checkers paralelo) con caché de gate por hash + max-age 24h, mueve `semgrep`/`pip-audit`/`pytest full` a CI authoritative manteniendo shift-left (CI ejecuta antes de merge; watch loop autofixea), paraleliza dispatch de docs con la ola de checkers, acota el watch loop por wall-clock (30 min active / 4h passive), emite `gate-findings.json` v1 como contrato hacia spec-103 (risk-accept), y poda 30% de verbosity quirúrgicamente eliminando solo duplicados verificados. Beneficio medible: warm-cache `/ai-pr` ≥40% más rápido, cold-cache ≤90s, watch loop nunca se cuelga en bucles ilimitados, una sola fuente CLI funciona idéntica para los cuatro IDEs.
...
- NG-1: `ai-eng risk accept` CLI y subcomandos (`accept`, `accept-all`, `renew`, `resolve`) — alcance spec-103 (S3).
- NG-2: Wiring gate↔risk-accept lookup en `policy/gates.py` y `policy/checks/*.py` — alcance spec-103.
- NG-3: Auto-fix `git add -u` post-`ruff format` (re-stage de archivos modificados por el fixer) — alcance spec-103 sub-feature `gates.pre_commit.auto_fix`.
- NG-4: `/skill-sharpen` global sweep sobre las 47 skills — alcance spec-104 (S4).
- NG-5: MCP Sentinel security audit de skills/agents/MCP servers — alcance spec-105 (S5).
...
- NG-10: Reescritura completa de `policy/gates.py`. S2 añade orchestrator y cache; el gate dispatch existente queda como fallback bajo `AIENG_LEGACY_PIPELINE=1`.

## Decisions

### D-102-01: Single-pass collector con dos olas (fixers serial → checkers paralelo)
...
- **Salida**: `.ai-engineering/state/gate-findings.json` con todos los failures de Wave 2 (Wave 1 falla solo si los fixers no convergen tras un re-run automático intra-wave). Schema en D-102-06.
- **Comportamiento en falla**: si Wave 2 produce findings con `severity >= medium`, `ai-eng gate run` exit 1 sin proceder a commit; spec-103 conectará risk-accept para permitir override consciente.
- **Fallback de emergencia**: `AIENG_LEGACY_PIPELINE=1` env restaura el flujo secuencial pre-S2; documentado en CHANGELOG migration.
...
### D-102-02: Local fast-slice + CI authoritative (política fija de framework)
...
### D-102-03: gate-cache con clave hash + max-age 24h

`src/ai_engineering/policy/gate_cache.py` (nuevo) implementa caché por check con las siguientes invariantes:

- **Cache key**: `sha256(tool_name ‖ tool_version ‖ sorted(staged_blob_shas) ‖ sorted(config_file_hashes) ‖ sorted(args))` (definición precisa en D-102-09).
...
### D-102-04: Async paralelo docs + pre-push (sync, no fire-and-forget)

`/ai-pr` step 6.5 (docs subagentes) y step 7 (pre-push gate Wave 2) ejecutan **simultáneamente** en lugar de secuencialmente:

- 3 tareas concurrentes lanzadas: docs Agent 1 (CHANGELOG+README), docs Agent 2 (docs-portal+quality-gate), pre-push Wave 2 (los 5 checkers paralelos de D-102-01).
...
### D-102-05: Watch loop bounded por wall-clock (active 30min / passive 4h)
...
- **On cap**:
  1. Emite `.ai-engineering/state/watch-residuals.json` con mismo schema que `gate-findings.json` (D-102-06) — findings residuales de checks que aún fallan.
...
Constraints:
- `rule_id` MUST ser estable (CVE-XXXX, semgrep rule-id, gitleaks rule-id, ruff code E501, ty error code). NUNCA mensaje humano (LESSONS S3 nota: "Risk-accept lookup debe ser por finding-id estable").
- `schema` field versionado; consumers (spec-103 risk-accept) leen y rechazan versiones desconocidas con mensaje accionable.
- Fixture JSON canónica en `tests/fixtures/gate_findings_v1.json` valida schema en CI.

**Rationale**: stable-id-keyed schema permite a spec-103 hacer lookup `finding_is_accepted(finding.rule_id, store)` determinístico. Versión explícita en `schema` field habilita evolución no-breaking (v2 puede añadir campos opcionales). Telemetría `wall_clock_ms` facilita medir impacto real de S2 post-merge sin instrumentación adicional.

### D-102-07: Verbosity reduction conservadora (solo duplicados verificados)
...
- `tests/unit/test_skill_line_budget.py` (nuevo) asserta line count post-S2 ≤ baseline pre-S2 menos 160.
...
### D-102-08: Cross-IDE parity vía CLI-layer caching
...
_CONFIG_FILE_WHITELIST = {
    ...
    # semgrep, pip-audit, pytest-full no aplican (no corren local en S2)
}
```
...
### D-102-10: Override flags y kill switch
...
- **R-4 — Schema drift gate-findings.json entre S2 y S3**. S3 podría ampliar el schema y romper consumers. *Mitigación*: `schema` field versionado; fixture canónica en `tests/fixtures/gate_findings_v1.json`; S3 brainstorm referenciará este spec y debe mantener back-compat o emitir v2 (additive only).
...
- **R-7 — Verbosity cuts rompen un IDE no-Claude**. Codex o Gemini consumían sección removida. *Mitigación*: `tests/unit/test_skill_contract_completeness.py` asserta secciones obligatorias mantenidas; `ai-eng sync --check` mandatory en CI; alcance D-102-07 limitado a duplicados verificados (no contenido único).
...
- **R-10 — spec-101 aún en flight en esta misma rama**. Conflictos en `manifest.yml` o `policy/checks/stack_runner.py`. *Mitigación*: alcance S2 ortogonal a archivos S1 (verificado: spec-101 NO toca `ai-commit/SKILL.md`, `ai-pr/SKILL.md`, `watch.md`, ni añade nada en `policy/orchestrator.py`/`gate_cache.py`); install-smoke CI continúa corriendo unchanged; `manifest.yml` add-only (sección `gates.policy_doc_ref` nueva, no modifica `required_tools`); rebase final si surge conflicto resuelve trivialmente.
...
- **R-14 — Watch loop emite watch-residuals.json mientras spec-103 risk-accept aún no existe**. User no puede ejecutar `ai-eng risk accept-all` referenciado en el mensaje. *Mitigación*: mensaje accionable incluye fallback "or fix manually and re-invoke `/ai-pr`"; spec-103 brainstorm (próximo) priorizará el CLI risk-accept para cerrar el loop; en este branch ambos S2 y S3 entregan juntos.

## References

- `.ai-engineering/notes/adoption-s3-unified-gate-risk-accept.md` — consumer del schema gate-findings.json v1.
- `.ai-engineering/specs/spec.md` — spec-101 (installer robustness) en la misma rama; S2 es ortogonal en file scope.
- `.ai-engineering/LESSONS.md`:
  - "Stable framework orchestration should not become per-project config by default" (D-102-02 fundamento de no-configurable).
  - "Elimination can be the simplification, not migration" (D-102-07 fundamento de cuts quirúrgicos).
  - "manifest.yml es la fuente de verdad absoluta" (D-102-08 fundamento de CLI-layer central).
- `.ai-engineering/contexts/python-env-modes.md` — spec-101 D-101-12 contrato worktree-friendly que `gate-cache` storage respeta.
- CLAUDE.md Don't #1-9 — never-weaken-gates intacto bajo D-102-02 (gate ejecuta, solo cambia el momento; CI es shift-left de prod).
- Brainstorm 2026-04-26 sesión `/ai-brainstorm` — Q1-Q8 decisiones documentadas en chat history.
...
- **OQ-3**: `watch-residuals.json` schema es idéntico a `gate-findings.json` v1; cuando watch loop emite findings de checks que solo corren en CI (semgrep, pip-audit), ¿el schema necesita campo `source: local|ci`? Tentative: añadir como campo opcional en schema v1.1 si spec-103 lo requiere; versionado lo permite sin breakage.
- **OQ-4**: ¿Debe orchestrator soportar resume tras crash mid-Wave-2? E.g., guardar estado parcial en `.ai-engineering/state/orchestrator-state.json` y skip checks ya completados al re-run. Default: NO en S2 (complica caché y debugging); el cache cubre el 95% del beneficio. Tracked para spec-103+ si surge necesidad real.
...
## Predecessor Note (2026-04-24, preserved for context)

**Discovery Date**: 2026-04-24
**Original Context**: Diagnóstico ai-engineering v4 adoption. `/ai-commit` medium-slow, `/ai-pr` con watch loop unbounded es el peor. Duplicación local↔CI masiva. Priorizamos S1 (installer robustness, spec-101) primero; este spec se redactó tras finalizar el brainstorm S2 el 2026-04-26.

**Original problem inventory** (verificado vigente al 2026-04-26):
- 532 líneas entre `.claude/skills/ai-commit/SKILL.md` (126), `ai-pr/SKILL.md` (221) y `ai-pr/handlers/watch.md` (185) — vigente.
- Toda la barra de quality tools (ruff, gitleaks, pytest, ty, pip-audit, semgrep) corre **local Y en CI** sin coordinación — vigente.
- `ai-eng spec verify` se llama dos veces en una sesión de `/ai-pr` — vigente.
- Dos subagentes `/ai-docs` **síncronos** bloquean el critical path — vigente.
- Cero memoización; cada re-run ejecuta todo from scratch — vigente.

Original quick wins propuestos (1) eliminar semgrep local, (2) async docs post-PR, (3) dedup `ai-eng spec verify`, (4) memoización gate-cache fueron absorbidos y ampliados en las decisions D-102-01 a D-102-10. La propuesta TTL 300s del quick win 4 fue reemplazada por hash + max-age 24h tras brainstorm Q3.

Brainstorm 2026-04-26 decisiones: scope B (plumbing + collect-fix-all UX local), local fast-slice + CI authoritative política fija, hash + max-age 24h, 2-wave fixers/checkers, sync paralelo docs+pre-push, watch wall-clock 30min/4h, verbosity conservadora 30%, holding-file en esta nota. Branch directive del usuario: no merge spec-101 separado; toda la serie S vive en `feat/spec-101-installer-robustness` hasta delivery conjunto.
