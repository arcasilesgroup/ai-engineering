...
- [ ] T-3.1: Rename `.github/agents/explore.agent.md` → `.github/agents/ai-explore.agent.md`; update front-matter `name: ai-explore` (agent: build)
- [ ] T-3.2: Update `scripts/sync_command_mirrors.py` `AGENT_METADATA["explore"]["name"]` from "Explorer" to "ai-explore" (agent: build)
- [ ] T-3.3: Create `.github/chatmodes/ai-explore.chatmode.md` con front-matter handler agent:ai-explore (agent: build)
- [ ] T-3.4: Update `src/ai_engineering/templates/project/GEMINI.md` line 110: `## Skills (44)` → `## Skills (__SKILL_COUNT__)`; if agent count header exists similar treatment (agent: build)
- [ ] T-3.5: Add `write_gemini_md(canonical_skills, canonical_agents)` function to `scripts/sync_command_mirrors.py`; wire into main sync flow (agent: build)
- [ ] T-3.6: Update `.claude/skills/ai-platform-audit/SKILL.md` — add Check 6 (agent naming consistency cross-IDE), Check 7 (GEMINI.md skill count freshness), Check 8 (generic instruction-file count scan) (agent: build)
- [ ] T-3.7: Run `uv run ai-eng sync` to regenerate mirrors with renamed agent + updated GEMINI.md + updated platform-audit (agent: build)
- [ ] T-3.8: Run `uv run ai-eng sync --check` exit 0 (agent: verify)
- [ ] T-3.9: Update CHANGELOG with `BREAKING-LIKELY: Copilot agent renamed Explorer → ai-explore` entry (agent: build)
- [ ] T-3.10: Write `tests/integration/test_copilot_explorer_rename.py` body — assert `name: ai-explore` in agent file, chatmode exists, mirror parity preserved (agent: build)
- [ ] T-3.11: Write `tests/unit/test_gemini_md_placeholders.py` body — load template, assert `__SKILL_COUNT__` and `__AGENT_COUNT__` placeholders present; load generated `.gemini/GEMINI.md`, assert canonical numeric counts substituted (agent: build)
- [ ] T-3.12: Write `tests/integration/test_platform_audit_new_checks.py` body — fixture projects con drifts intentionales (renamed agent name mismatch slug, GEMINI.md count stale, CLAUDE.md count stale); assert each new check flags correctly (agent: build)
- [ ] T-3.13: Remove markers from these 3 test files; confirm GREEN (agent: build)
- [ ] T-3.14: Write RED skeleton `tests/integration/test_sentinel_runtime_iocs.py` marked, covering Phase 4 G-8 (agent: build)
- [ ] T-3.15: Write RED skeleton `tests/integration/test_sentinel_risk_accept.py` marked, covering Phase 4 G-9 (agent: build)
- [ ] T-3.16: Run pytest filter; confirm PASS (agent: verify)
- [ ] T-3.17: Stage and commit `feat(spec-107): Phase 3 GREEN Explorer rename + GEMINI placeholders + platform-audit + Phase 4 RED tests` (agent: build)
