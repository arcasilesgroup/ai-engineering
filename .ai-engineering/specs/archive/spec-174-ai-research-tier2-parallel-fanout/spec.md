...
Results from all providers are merged and deduped by exact URL string (no URL
normalization is applied — the `url` value is compared verbatim). On a duplicate
URL, the row from the higher-priority provider is kept and the lower ones
dropped; priority order is **Tavily > Exa > built-in WebSearch**. Each hit
