# Proposed: `.claude/agents/ai-evaluator.md`

T-2.1 produced this agent body. Auto-mode harness denied the autonomous write into `.claude/agents/` (the dispatch-surface directory is protected against autonomous self-modification of the harness — a deliberate safety net, not a pipeline regression). Apply this file content to `.claude/agents/ai-evaluator.md` during a maintainer review pass; the agent body below is final.

The deferral does not stop the runtime engine landing — `src/ai_engineering/eval/` is fully implemented and tested under Phase 1+2. Once the agent file is in place, the dispatcher can call `subagent_type: ai-evaluator` and the agent body below tells it exactly what to do.
...
- **Build Context Output Contract** -- the Findings / Dependencies / Risks / Recommendations block produced by `ai-build`, plus the deliverable hand-off shape: file paths produced, test-runner entry points, runnable scenario hooks. See `.claude/agents/ai-build.md` Context Output Contract.
