...
- [ ] T-1 — Rewrite `test_ai_research_tier2.py` for the fan-out contract
  - Agent: build
...
- [ ] T-2 — Replace the cascade+fall-through in `_ai_research_tier2_helper.py` with fan-out
  - Agent: build
...
- [ ] T-3 — Rewrite `tier2-web.md` algorithm + status to fan-out
  - Agent: build
...
- [ ] T-4 — Update `SKILL.md` Tier-2 wording + example
  - Agent: build
...
- [ ] T-5 — Regenerate mirror + template surfaces via dev sync
  - Agent: build
...
- [ ] T-6 — Full test + parity verification
  - Agent: verify
...
- `test_ai_research_resilience.py` imports `tier2_web` but asserts Tier-1/Tier-3
  degraded sources (context7/ms_learn/gh_search/notebooklm), not the web
  cascade — expected to stay green unchanged; T-6 confirms.
