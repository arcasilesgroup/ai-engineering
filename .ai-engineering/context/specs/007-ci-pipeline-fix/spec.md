...
2. **Content integrity (error 13)**: `uv run ai-eng validate` reports 10 broken file references across 6 governance files. These reference deleted directories (`context/backlog/`, `context/delivery/`) and moved files (`skills/swe/create-skill.md` → `skills/lifecycle/create-skill.md`). The validator exits with code 1 on any `FAIL`.
