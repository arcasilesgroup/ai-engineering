...
2. If rebase conflicts: READ `.agents/skills/ai-resolve-conflicts/SKILL.md` and delegate to its category-aware logic (lock files regenerate; migrations ask user; generated files accept theirs; config AI-merge; code intent-aware). Keeps lock-file regeneration, migration ordering, and stacked-PR detection active.
