...
2. **Dependency preflight** — verify `.agents/skills/ai-verify/handlers/verify.md` plus required `.agents/agents/internal/verifier-*.md` files exist for the selected mode (`normal` and `--full` both require deterministic + acceptance post-W3; individual modes require only the matching specialist). STOP and report exact missing path(s) — never improvise.
...
Dispatch the `ai-verify` agent for any merge-readiness check, scan, or evidence-backed claim. Hand off via `Agent` tool — each specialist runs in its own context window. The agent file (`.agents/agents/ai-verify.md`) is the orchestrator handle; the procedural contract — modes, profiles, output contract — lives here.
