...
**Calls**: the `ai-simplify` agent (`.agents/agents/ai-simplify.md`) via the Agent tool with the operator-chosen scope; validation runs after each edit; the agent rolls back on test failure.

**See also**:
- `.agents/skills/ai-simplify-sweep/SKILL.md` — scheduled wrapper with draft-PR side effect.
