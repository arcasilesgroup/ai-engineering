...
2. **Tier 3 launch (T0, background, default-on)** -- follow the Launch half of `handlers/tier3-notebooklm.md`. The `mcp__notebooklm__nlm_list` probe runs INSIDE the background subagent as the availability gate (D-172-11) -- before any `nlm_create_notebook`/`nlm_research`; on unavailable/raise the subagent degrades at T0 (a warning, no blocking banner) and the main agent proceeds on Tiers 0-2 (fail-soft D-172-09). When NotebookLM is available, launch its autonomous deep-research job FIRST in a background subagent (`nlm_create_notebook` then `nlm_research(mode=deep)`, which is NON-blocking and returns an immediate `status="in_progress"` ack) so it overlaps the fast tiers. This runs **by default with no flag**; when NotebookLM is absent/unauthenticated the launch short-circuits to degraded (recorded, never raised) and the run proceeds on Tiers 0-2.
...
6. **Tier 3 harvest (bounded wait)** -- follow the Harvest half of `handlers/tier3-notebooklm.md`. After Tiers 0-2 finish, poll the deep-research job STATUS until a terminal state, bounded by `AIENG_RESEARCH_NLM_WAIT_SEC` (default 300s, ceiling 900s) with a capped per-poll back-off (`AIENG_RESEARCH_NLM_POLL_INTERVAL_SEC`, default 5s, ceiling 60s). On `completed`, collect the deep report (`report_markdown`) and the sources NotebookLM discovered autonomously. On timeout/failure (`failed`/`error`/`[AUTH_REQUIRED]`/budget exceeded), degrade gracefully and preserve `notebook_id` for `--reuse-notebook` so a later `--reuse-notebook=<id>` harvests the finished report.
