...
| `uv run ai-eng spec new SLUG [--ref owner/repo#45]` | Crea `specs/NNN-slug/spec.md`; `--ref` intenta sembrarlo desde GitHub o Azure Boards. |
...
| `uv run ai-eng accept --finding ID --expires AAAA-MM-DD [--severity NIVEL] [--by PERSONA] [--justification TEXTO] [--follow-up TEXTO] [--spec ID]` · `uv run ai-eng accept --expired` | Registra un riesgo con dueño y caducidad, o lista los ya caducados. |
