...
| `uv run ai-eng decide "DECISION" [--supersede NNNN]` · `uv run ai-eng decide --list` · `uv run ai-eng decide --accept NNNN` | Creates or lists a MADR in `docs/adr/`: it promotes a decision marked `[X]` under `## Decisions` in its spec into a proposed record named for its title — proposal is not approval, and the spec keeps its own unmarked decisions. `--list` shows the MADRs. `--supersede NNNN` marks which ADR a new one replaces. `--accept` moves a MADR out of `proposed` taking the authority of the approved Solution Intent; it does not invent it or ask again, and if the Intent does not validate or is still draft it grants nothing. The commit is yours. |
