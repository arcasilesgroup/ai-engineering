...
Read the target skill's SKILL.md. If `$ARGUMENTS` is `all`, list skills from `.gemini/skills/` and process them in priority order: workflow skills first (plan, dispatch, review, verify, commit, pr), then enterprise, then meta.
