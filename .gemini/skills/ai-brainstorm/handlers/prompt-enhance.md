...
Apply two `/ai-prompt-tune` techniques to the input. The catalogue is the single source of truth in `.gemini/skills/ai-prompt-tune/SKILL.md` -- do not duplicate it here.

- **Technique #1 -- Be Explicit Over Implicit** -- see section "1. Be Explicit Over Implicit" in `.gemini/skills/ai-prompt-tune/SKILL.md`. Replace vague verbs with specific actions; add missing specificity that is inferable from the term itself (no codebase reading). Name the type of change (`add`, `remove`, `replace`, `redesign`, `extract`, `split`).
- **Technique #5 -- Positive Framing** -- see section "5. Positive Framing" in `.gemini/skills/ai-prompt-tune/SKILL.md`. Rewrite negations as positive targets. State what to build, not what to avoid.
