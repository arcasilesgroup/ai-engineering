...
- Sub-spec directories exist at `.ai-engineering/state/runtime/autopilot/sub-NNN/` with `spec.md` (Scope + `files:` frontmatter) and `plan.md` (Plan placeholder).
- Manifest exists at `.ai-engineering/state/runtime/autopilot/manifest.md` with all sub-spec statuses set to `planning`.
...
1. Glob `.ai-engineering/state/runtime/autopilot/sub-*/spec.md`. Collect the full list of sub-spec directories.
...
- N enriched sub-spec directories at `.ai-engineering/state/runtime/autopilot/sub-NNN/`:
