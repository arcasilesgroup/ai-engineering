...
def generate_opencode_plugin() -> str:
    """Generate ``.opencode/plugin/ai-engineering.ts`` (guard-plane entry).

    OpenCode 1.18.5 auto-discovers any ``*.ts``/``*.js`` under
    ``.opencode/plugin/`` — no ``opencode.json`` entry is required, so none is
    created (adding one would be a seventh unmanaged root config with no
    generator). The entry is deliberately a thin re-export: all guard logic
    lives in the canonical, twin-synced
    ``.ai-engineering/scripts/hooks/opencode-hook-bridge.ts`` so there is
    exactly one place to edit when the plugin contract moves.

    The relative import holds in both layouts the file ships to: the repo root
    (``<repo>/.opencode/plugin/`` -> ``<repo>/.ai-engineering/...``) and a
    consumer install, where the installer copies the whole ``.opencode`` tree
    and the whole ``.ai-engineering/scripts`` tree side by side.
    """
    return (
        "// GENERATED by `ai-eng dev sync`"
        " (scripts/sync_mirrors/opencode_target.py::generate_opencode_plugin).\n"
