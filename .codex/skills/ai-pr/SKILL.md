...
Runs immediately after Step 14 creates the PR (so the PR number `N` is known) and BEFORE Step 15 enables auto-complete. **Skip for `--draft` PRs** — a draft is not heading to merge, so consolidating would mark the spec SHIPPED while the draft sits open; consolidate when the draft is promoted (`/ai-pr review` / `--update` with auto-complete).

If `.ai-engineering/specs/spec.md` is non-placeholder, resolve the canonical numeric id from its frontmatter `spec:` field (e.g. `spec-153` — NOT the human-readable slug; numeric `spec-NNN` is the one canonical identity per spec-153 D-153-01) and run the load-bearing call `python .ai-engineering/scripts/spec_lifecycle.py mark_shipped <spec-NNN> N <branch>` **directly on the feature branch** (it walks the state forward to SHIPPED, appends the canonical 7-col `_history.md` row, snapshots spec.md+plan.md into `archive/spec-NNN-<slug>/`, clears spec.md+plan.md to placeholders, and emits the `framework_operation` audit event). Do NOT route this through the `_shared/consolidate-spec.md` handler: that handler's SHIPPED-precondition guard (refuse-if-not-SHIPPED) is correct for the manual/backstop path but would reject the pre-merge spec, which is still IN_PROGRESS here — and marking it SHIPPED as the ship-PR opens is precisely the intent. Then `git add` the archived + cleared files, commit `chore(spec-NNN): consolidate (archive + clear live slot)`, and push to the same branch so the open PR updates.

**Fail-open**: a lifecycle write failure logs but never blocks the PR. Because consolidation now rides the feature PR, it lands regardless of whether the PR is squash-merged via `gh`/`az` or the GitHub web UI — there is NO separate follow-up chore PR. The same handler stays exposed manually via `/ai-pr --consolidate-spec`. `/ai-branch-cleanup` Phase 5 `reconcile_merged` is now a defense-in-depth no-op backstop that only acts if this step was skipped or failed (e.g. a draft that merged without promotion).
