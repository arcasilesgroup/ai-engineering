...
| `<repo>/.opencode/skills/ai-<name>/SKILL.md` | (ACTIVE per spec-128 Wave 4, supersedes D-133-06) Native OpenCode on-demand skills (folder per skill). Generated by `scripts/sync_mirrors/opencode_target.py`. | Hand-edited content. Skills under `.opencode/commands/` (saved-prompt path; framework writes nothing there). |
...
| `<repo>/.opencode/commands/*` | (NOT USED for skills — framework emits skills only under `.opencode/skills/`. Operators may add their own slash commands here without conflict.) | Framework-generated content. |
